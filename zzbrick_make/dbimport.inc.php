 ...
 * @return array
 */
function mod_dbtransfer_make_dbimport() {
	$data = [];

	if ($_SERVER['REQUEST_METHOD'] === 'POST' AND empty($_GET['table'])) {
		if (!empty($_POST['delete'])) {
			$first = key($_POST['delete']);
			$filename = sprintf('%s/dbtransfer/dbimport_ids-%s.log', wrap_setting('log_dir'), key($_POST['delete']));
			if (file_exists($filename)) unlink($filename);
			wrap_redirect_change();
		} elseif (!empty($_POST['import'])) {
			$data = mod_dbtransfer_make_dbimport_go(key($_POST['import']));
		}
	} else {
		...
		foreach ($log as $line) {
			if (!array_key_exists($line['table'], $data))
				$data[$line['table']] = [
					'table' => $line['table'],
					'records' => 1,
					'logged' => mod_dbtransfer_dbimport_log($line['table'], 'count')
				];
			else
		...
		else $data = mod_dbtransfer_dbimport_table($data, $log);
	}
...
function mod_dbtransfer_dbimport_table($data, $log) {
	ini_set('max_execution_time', 0);

	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		mod_dbtransfer_dbimport_table_save();
	}
	...
	$sql = mf_dbtransfer_record_sql($data['table'], $conditions);
	
	$data['relations'] = [];
	$relations = mf_dbtransfer_relations();
	...
	foreach ($tabledata as $record_id => $line) {
		if (!array_key_exists($record_id, $existing)) {
			mod_dbtransfer_dbimport_log($data['table'], 'write', $record_id);
			$data['new']++;
			continue;
		}
		mod_dbtransfer_dbimport_diff($data, $record_id, $line, $existing[$record_id]);
	}
 ...
 */
function mod_dbtransfer_dbimport_log($table, $action, $old_record_id = 0, $new_record_id = 0) {
	static $increment = [];
	static $log = [];
 ...
 */
function mod_dbtransfer_dbimport_diff(&$data, $record_id, $record, $record_existing) {
	static $unique_fields = [];
	// check: already logged?
	$new_record_id = mod_dbtransfer_dbimport_log($data['table'], 'check', $record_id);
	...
	if ($record === $record_existing) {
		mod_dbtransfer_dbimport_log($data['table'], 'write', $record_id, $record_id);
		$data['identical']++;
		return;
	}
	
	// check: unique fields different? if yes, record is different
	if (wrap_setting('dbtransfer_import_diff_unique_fields')) {
		if (!$unique_fields) {
			foreach (wrap_setting('dbtransfer_import_diff_unique_fields') as $field) {
				$field = explode('.', $field);
	...
	if (in_array($data['table'], wrap_setting('dbtransfer_import_diff_mntables'))) {
		mod_dbtransfer_dbimport_log($data['table'], 'write', $record_id);
		$data['new']++;
		return;	
	}
 ...
 *
 */
function mod_dbtransfer_dbimport_table_save() {
	if (array_key_exists('identical', $_POST))
		mod_dbtransfer_dbimport_log($_GET['table'], 'write', $_POST['record_id'], $_POST['record_id']);
	else
		mod_dbtransfer_dbimport_log($_GET['table'], 'write', $_POST['record_id']);
 ...
 * @return array
 */
function mod_dbtransfer_make_dbimport_go($table) {
	$relations = mf_dbtransfer_relations();
	$fields = [];
	$ids = [];
	$id_field = '';
	...
	if (mod_dbtransfer_make_dbimport_translationtable($table)) {
		$t_fields = mf_dbtransfer_translationfields();
		$t_tables = [];
	...
	foreach ($fields as $related_table) {
		if (!array_key_exists($related_table, $ids))
			$ids[$related_table] = mod_dbtransfer_make_dbimport_ids($related_table);
	}
	...
	foreach (wrap_setting('dbtransfer_import_identifiers') as $identifier) {
	...
	$data['imported'] = 0;
	$logfile = wrap_file_log('dbtransfer/dbexport');
	...
	foreach ($logfile as $line) {
		...
		foreach ($line['record'] as $field_name => $value) {
			...
			if ($field_name === $identifier_field_name) {
				...
				if (in_array($value, $identifiers))
					$value = sprintf('%s%s', $value, wrap_setting('dbtransfer_import_identifiers_suffix'));
			}
			if ($value AND array_key_exists($field_name, $fields)) {
				if (mod_dbtransfer_make_dbimport_translationtable($table) AND $field_name === 'translationfield_id') {
					// don’t do anything
			...
			if (mod_dbtransfer_make_dbimport_translationtable($table) AND $field_name === 'field_id') {
		...
		zz_db_log($sql, wrap_setting('dbtransfer_import_log_username'), $id);
		$data['imported']++;
	}
	$data['table'] = $table;
	return $data;
 ...
 * @return array
 */
function mod_dbtransfer_make_dbimport_ids($table) {
	static $data = [];
	if (array_key_exists($table, $data)) return $data[$table];

	$log = wrap_file_log(sprintf('dbtransfer/dbimport_ids[%s]', $table));
	$data[$table] = [];
	foreach ($log as $line)
 ...
 * @return bool
 */
function mod_dbtransfer_make_dbimport_translationtable($table) {
